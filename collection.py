...
for item_id in items:
    ...
    with open(f'{html_dir}/{title}_details.html', 'w', encoding='utf-8') as details_file:
        ...
        details_file.write(f'<div style="display:flex;">')
        details_file.write(f'<div style="flex:1;"><img src="{image_file}" style="height: 100vh;"></div>')
        details_file.write(f'<div style="flex:1;padding: 20px;">')
        details_file.write(f'<h1>{title}</h1>')
        details_file.write(f'<table>')
        details_file.write(f'<tr><td>Min Players</td><td>{minplayers}</td></tr>')
        ...
        details_file.write(f'<tr><td>Description</td><td>{description}</td></tr>')
        details_file.write(f'</table>')
        details_file.write(f'</div>')
        details_file.write(f'</div>')
